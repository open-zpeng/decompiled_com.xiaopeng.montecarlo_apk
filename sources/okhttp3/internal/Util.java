...
public final class Util {
    ...
    public static String hostHeader(HttpUrl httpUrl, boolean z) {
        String host;
        if (httpUrl.host().contains(LocationUtils.DR_RECORD_VALUE_SPLIT_CHAR)) {
            host = "[" + httpUrl.host() + "]";
        } else {
            host = httpUrl.host();
        }
        if (z || httpUrl.port() != HttpUrl.defaultPort(httpUrl.scheme())) {
            return host + LocationUtils.DR_RECORD_VALUE_SPLIT_CHAR + httpUrl.port();
        }
        return host;
    }
    ...
    private static String inet6AddressToAscii(byte[] bArr) {
        ...
        while (i < bArr.length) {
            if (i == i3) {
                ...
                buffer.writeHexadecimalUnsignedLong(((bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8) | (bArr[i + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR));
                i += 2;
            }
        }
