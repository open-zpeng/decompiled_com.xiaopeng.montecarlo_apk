...
class Huffman {
    ...
    public void encode(ByteString byteString, BufferedSink bufferedSink) throws IOException {
        ...
        while (i < byteString.size()) {
            int i2 = byteString.getByte(i) & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR;
            int i3 = CODES[i2];
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i = 0; i < byteString.size(); i++) {
            j += CODE_LENGTHS[byteString.getByte(i) & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR];
        }
        return (int) ((j + 7) >> 3);
    }
    ...
    public byte[] decode(byte[] bArr) {
        ...
        int i2 = 0;
        for (byte b : bArr) {
            i = (i << 8) | (b & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
            i2 += 8;
            while (i2 >= 8) {
