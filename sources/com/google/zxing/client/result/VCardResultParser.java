...
public final class VCardResultParser extends ResultParser {
    ...
    private static final Pattern SEMICOLON = Pattern.compile(LocationUtils.DR_RECORD_DATA_SPLIT_CHAR);
            ...
            if (indexOf < 0) {
                ...
                if (indexOf > end) {
                    ...
                    if (i != 0) {
                        ...
                        if (z2) {
                            replaceAll = UNESCAPED_SEMICOLONS.matcher(replaceAll).replaceAll(FDManager.LINE_SEPERATOR).trim();
                        }
                    } else {
                        if (z2) {
                            substring = UNESCAPED_SEMICOLONS.matcher(substring).replaceAll(FDManager.LINE_SEPERATOR).trim();
                        }
                        replaceAll = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(substring).replaceAll("")).replaceAll(FDManager.LINE_SEPERATOR)).replaceAll("$1");
                    }
                    if (IntentActions.ActionsKey.KEY_URI.equals(str3)) {
                        try {
