...
public class dumpcrash {
    ...
    private static native void setLogSwitch(int i);
    ...
    public static void setAppFileDir() {
        setFileDir(CrashLog.getApplication().getFilesDir() + File.separator);
    }

    public static synchronized void loadLibrary() {
        synchronized (dumpcrash.class) {
            LogUtil.log(1, "dumpcrash loadLibrary");
            if (hasLoadLibrary) {
                return;
            }
            AutoLibraryLoader.loadLibrary(LIB_DUMPCRASH);
            hasLoadLibrary = true;
            LogUtil.log(1, "dumpcrash loadLibrary hasLoadLibrary = " + hasLoadLibrary);
        }
    }
    ...
    private static String getJavaStackTrace(Thread thread, Thread thread2, StackTraceElement[] stackTraceElementArr) {
        if (stackTraceElementArr == null || stackTraceElementArr.length == 0) {
            if (thread != null) {
                LogUtil.log("dc: TN: " + thread.toString());
            }
            if (stackTraceElementArr == null) {
                LogUtil.log("dc: element null");
                return "";
            }
            LogUtil.log("dc: element length 0");
            return "";
        }
