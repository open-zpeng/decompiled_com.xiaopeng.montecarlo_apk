...
public class SoCollector {
    ...
    public static synchronized void removeOtherTamerFixLibPath(Context context) {
        synchronized (SoCollector.class) {
            ...
            LogUtil.logE("BASE_APK_VERSION = " + BASE_APK_VERSION + ",BASE_LIB_PATH" + BASE_LIB_PATH);
            if (!TextUtils.isEmpty(BASE_APK_VERSION) && !TextUtils.isEmpty(BASE_LIB_PATH)) {
                ...
                if (file.exists()) {
                    ...
                    for (File file2 : listFiles) {
                        LogUtil.logE("removeFile :" + file2.getName());
        ...
        synchronized (SoCollector.class) {
            if (TextUtils.isEmpty(BASE_APK_VERSION)) {
                ...
                try {
                    ...
                    LogUtil.logE("get version name error");
                }
            }
        }
    }
    ...
    private static Map<String, SoFileEntry> dumpInApkLibsByJava(Context context) {
        ...
        try {
            ...
            LogUtil.logE("start dumpInApkLibsByJava");
                ...
                if (!nextElement.isDirectory()) {
                    ...
                    if (!name.startsWith("..") && !name.startsWith(".") && name.length() >= 4 && name.endsWith(LIB_SUFFIX) && name.startsWith(APK_LIB) && (lastIndexOf = name.lastIndexOf(47)) >= 0) {
                        ...
                        if (name.regionMatches(i, LIB_PREFIX, 0, 3)) {
                            ...
                            LogUtil.logE("dumpInApkLibsByJava entryName = " + name + ",libFileName = " + substring);
                            SoFileEntry soFileEntry = new SoFileEntry();
                            ...
                            LogUtil.logE("dumpInApkLibsByJava entryName = " + name + ",libFileName = " + substring + ",fileLength = " + soFileEntry.fileLength + ",crc32 = " + soFileEntry.crc32);
            ...
            zipFile.close();
            LogUtil.logE("end dumpInApkLibsByJava time :" + (currentTimeMillis2 - currentTimeMillis));
        } catch (Exception e) {
            LogUtil.logE("dumpInApkLibsByJava error " + e.getMessage());
        }
        return hashMap;
    }
    ...
    */
    ...
    /* JADX WARN: Removed duplicated region for block: B:95:0x029b A[Catch: all -> 0x02bb, TRY_LEAVE, TryCatch #11 {all -> 0x02bb, blocks: (B:93:0x027d, B:95:0x029b, B:21:0x003b), top: B:107:0x003b }] */
    ...
    */
    ...
    public static String getSysLibSoInfo() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (file.exists()) {
                LogUtil.logE(SYSTEM_LIBS[i] + " exists");
    ...
    public static void dumpInApkLibs(TamperLibraryCallBack tamperLibraryCallBack, Context context, long j, String str) {
        if (context == null) {
            Application application = CrashLog.getApplication();
            if (application == null) {
                LogUtil.logE("dumpInApkLibs error as application = null");
        ...
        if (file2.exists()) {
            LogUtil.logE("has filedir and delete");
            IOUtil.deleteFileOrDir(file2);
        }
        file2.mkdirs();
        try {
            long usableSpace = file2.getUsableSpace();
            LogUtil.logE("libpath = " + file2 + ",usableSpace = " + usableSpace);
            if (usableSpace < j) {
            ...
            LogUtil.logE("dumpInApkLibs error as usableSpace exception");
            tamperLibraryCallBack.complete(2);
        }
    }
    ...
    public static boolean copyLibFromApkToDest(String str, boolean z, Context context, String str2) {
                                ...
                                if (!name.startsWith("..") && !name.startsWith(".") && name.length() >= 4 && name.endsWith(LIB_SUFFIX) && name.startsWith(APK_LIB) && (lastIndexOf = name.lastIndexOf(47)) >= 0) {
                                    ...
                                    if (name.regionMatches(i, LIB_PREFIX, 0, 3)) {
                                        ...
                                        if (z2 || TextUtils.isEmpty(str3) || str3.equals(substring)) {
                                            LogUtil.logE("entryName = " + name + ",libFileName = " + substring);
                                            ...
                                            try {
                                                ...
                                                e = e;
                                                fileOutputStream2 = fileOutputStream3;
                                                LogUtil.logE("dump file error " + e.getMessage());
                                                if (inputStream != null) {
                                                    try {
                                                        inputStream.close();
                                                    } catch (Exception unused) {
                                                ...
                                                long currentTimeMillis2 = System.currentTimeMillis();
                                                LogUtil.logE("all copy time = " + (currentTimeMillis2 - currentTimeMillis));
                                                return z3;
                                            } catch (Throwable th) {
                                                th = th;
        ...
        LogUtil.logE("all copy time = " + (currentTimeMillis22 - currentTimeMillis));
        return z3;
    }
}
