...
public class LogUtils {
    ...
    private static void logHeaderContent(int i, int i2) {
        ...
        while (i2 > 0) {
            if (i2 + stackOffset < stackTrace.length) {
                str = str + "   ";
                logByLevel(i, HORIZONTAL_LINE + ' ' + str + getSimpleClassName(stackTrace[i3].getClassName()) + "." + stackTrace[i3].getMethodName() + "  (" + stackTrace[i3].getFileName() + LocationUtils.DR_RECORD_VALUE_SPLIT_CHAR + stackTrace[i3].getLineNumber() + ")");
            }
            i2--;
        }
    }
