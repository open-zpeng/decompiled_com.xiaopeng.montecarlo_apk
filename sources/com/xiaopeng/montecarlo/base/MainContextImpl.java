...
public class MainContextImpl implements IMainContext {
    ...
    public void updateRecenterBtn() {
        this.mMapFragment.mo96getPresenter().updateRecenterStatus();
    }
    ...
    public void switchMapModeForVui(DrawModeType drawModeType) {
        MapScenePresenter mo96getPresenter = this.mMapFragment.mo96getPresenter();
        if (mo96getPresenter != null) {
            if (drawModeType == null) {
                mo96getPresenter.switchMapMode();
            } else if (drawModeType == DrawModeType.NORTHUP) {
                mo96getPresenter.executeSpeechCmdMapModeNorthUp();
            } else if (drawModeType == DrawModeType.HEADUP_2D) {
                mo96getPresenter.executeSpeechCmdMapModeHeadUp2D();
            } else if (drawModeType == DrawModeType.HEADUP_3D) {
                mo96getPresenter.executeSpeechCmdMapModeHeadUp3D();
            }
        }
    }
    ...
    public void switchMapScaleForVui(int i) {
        if (i == 19) {
            this.mMapFragment.mo96getPresenter().zoomInToMax();
        } else if (i == 3) {
            this.mMapFragment.mo96getPresenter().zoomOutToMin();
        } else {
            this.mMapFragment.mo96getPresenter().zoom(i);
        }
    }
    ...
    public void showLocationInfo(final boolean z) {
        runOnUiThreadRelyMap(new Runnable() { // from class: com.xiaopeng.montecarlo.base.MainContextImpl.2
            ...
            public void run() {
                MainContextImpl.this.mMapFragment.mo96getPresenter().showLocationInfo(z);
            }
        });
    }
    ...
    public void showSRData(final boolean z) {
        runOnUiThreadRelyMap(new Runnable() { // from class: com.xiaopeng.montecarlo.base.MainContextImpl.3
            @Override // java.lang.Runnable
            public void run() {
                MainContextImpl.this.mMapFragment.mo96getPresenter().showSRData(z);
            }
        });
    }
    ...
    public boolean isLocationInfoShow() {
        return this.mMapFragment.mo96getPresenter().isLocationInfoShow();
    }
    ...
    public void onChargeSettingChanged(boolean z) {
        this.mMapFragment.mo96getPresenter().setChargeEnable(z);
    }

    @Override // com.xiaopeng.montecarlo.base.IMainContext
    public void onScratchSpotChanged(boolean z) {
        this.mMapFragment.mo96getPresenter().setScratchSpotEnable(z);
    }
