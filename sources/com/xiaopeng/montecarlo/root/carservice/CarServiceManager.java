...
public class CarServiceManager implements ICarService.ICarStateListener {
    ...
    public interface IXPCarStateListener {
        ...
        default void onImuRawDataChange(boolean z, boolean z2, float[] fArr) {
        }
    ...
    public int getCarType() {
        if (!isInitialized()) {
            L.e(TAG, "getCarType but is not initialized!");
            return 0;
        }
        if (29947 == this.mCarType) {
            final Context context = ContextUtils.getContext();
            String hardwareCarType = getHardwareCarType();
            if ("D10".equals(hardwareCarType)) {
                this.mCarType = 1;
            } else if ("D20".equals(hardwareCarType)) {
            ...
            } else if ("D22".equals(hardwareCarType)) {
                return 10;
            } else {
                if ("D55".equals(hardwareCarType)) {
                    this.mCarType = 6;
                } else if ("E28".equals(hardwareCarType)) {
                    ...
                    this.mCarType = 9;
                } else if ("E38".equals(hardwareCarType)) {
                    this.mCarType = 8;
    ...
    public synchronized XPolitDriveMode getXPolitDriveMode() {
        if (this.mCarServiceWrapper != null) {
            if (this.mCarType == 0) {
            ...
            if (CarFeatureManager.INSTANCE.isACCLCCD2X()) {
                ...
                L.Tag tag = TAG;
                L.i(tag, "getXPolitDriveMode accStatus = mXPolitDriveMode = " + this.mXPolitDriveMode + ", accStatus = " + isAccOnByStatus + ", lccStatus = " + isLccOnByXPilotStatus);
            }
    ...
    public void onImuRawDataChange(boolean z, boolean z2, float[] fArr) {
        for (IXPCarStateListener iXPCarStateListener : this.mListeners) {
            iXPCarStateListener.onImuRawDataChange(z, z2, fArr);
        }
    }
