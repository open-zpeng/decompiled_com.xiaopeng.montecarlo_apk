...
public class GuideDataManager {
    ...
    public XPCameraInfo getCurrentCameraByNavCamera() {
        ...
        if (list != null) {
            for (XPCameraInfo xPCameraInfo : list) {
                if (xPCameraInfo != null && (7 == xPCameraInfo.mType || 22 == xPCameraInfo.mType)) {
    ...
    private void updateRouteContent(int i) {
        ...
        if (-1 == this.mCurrentNaviId || (routeData = TBTManager.getInstance().getRouteData(this.mCurrentNaviId)) == null || routeData.mRouteResult == null) {
            return;
        }
        int size = routeData.mRouteResult.mRouteInfos.size();
