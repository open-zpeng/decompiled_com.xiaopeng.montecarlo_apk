...
public class TBTManager {
    ...
    private int mCurrentDayNight = 0;
    ...
    public boolean init(@NonNull Application application, boolean z) {
        ...
        if (this.mPosServiceWrapper.initPos(z, this.mCarServiceManager.getCarType(), CarFeatureManager.INSTANCE.isAutoPolitIMUEnable())) {
            ...
            if (this.mRouteManager.initRouteService() && this.mGuideManager.initGuideService(RootUtil.SD_CARD_DIR_GUIDE)) {
                if (this.mBlAosServiceManager == null) {
                    this.mBlAosServiceManager = (BLAosServiceManager) ServiceMgr.getServiceMgrInstance().getBLService(15);
                }
    ...
    private void initTbtCommonComponent() {
        ...
        workPath.cache = str + SSUtils.O00000o0;
        workPath.navi = str + "navi";
        workPath.res = str + "res";
        UserConfig userConfig = new UserConfig();
        userConfig.deviceID = NavCoreUtil.getDiu();
    ...
    public synchronized void setCurrentStatus(int i) {
        ...
        if (this.mGuideManager != null) {
            this.mGuideManager.updateTBTStatus(i);
        }
    ...
    public void startNavi(long j, int i, int i2, int i3) {
        if (this.mGuideManager == null) {
            return;
        }
    ...
    public void stopNavi(boolean z) {
        if (this.mGuideManager != null) {
            if (3 == this.mCurrentStatus) {
                this.mGuideManager.onNaviStop(isSimulateGuidance() ? 1 : 0, false);
    ...
    public void setCruiseBroadcast(int i) {
        GuideManager guideManager = this.mGuideManager;
        if (guideManager != null) {
            guideManager.setCruiseBroadcast(i);
        }
    }
    ...
    public void setCurrentDayNight(int i) {
        if (L.ENABLE) {
            L.Tag tag = sTAG;
            L.d(tag, L.formatDayNightSwitchLog("setCurrentDayNight dayNight:" + i));
        }
        this.mCurrentDayNight = i;
    }

    public boolean isDayMode() {
        return 1 == this.mCurrentDayNight;
    }

    public void onSwitchTheme(boolean z) {
        this.mCurrentDayNight = z ? 1 : 2;
