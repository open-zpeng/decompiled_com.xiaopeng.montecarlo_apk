...
import com.xiaopeng.montecarlo.navcore.xptbt.bean.NgpRangetypeBroadcastPlaylistBean;
import com.xiaopeng.montecarlo.navcore.xptbt.bean.NgpRangetypeBroadcastStyleBean;
import com.xiaopeng.montecarlo.navcore.xptbt.bean.NgpRangetypeBroadcastTypeBean;
...
public class TTSManager implements CarServiceManager.IXPCarStateListener {
    ...
    private static final String NGP_RANGETYPE_BROADCAST_DETAIL_TYPE = "type_ngp_rangetype_detail";
    private static final String NGP_RANGETYPE_BROADCAST_JSON_DATA = "ngp_rangetype_broadcast_style.json";
    private static final String NGP_RANGETYPE_BROADCAST_SIMPLE_TYPE = "type_ngp_rangetype_simple";
    public static final int NORMAL_BROADCAST_MODE = 2;
    private static final String OFFLINE_RANGETYPE_FILENAME = "rangetype.json";
    ...
    private static NgpRangetypeBroadcastStyleBean sNgpRangetypeBroadcastStyleBean;
    private static List<NgpRangetypeBroadcastTypeBean> sNgpRangetypeDetailList;
    private static List<NgpRangetypeBroadcastTypeBean> sNgpRangetypeSimpleList;
    ...
    private static boolean readConfigFile() {
        ...
        NgpRangetypeBroadcastStyleBean ngpRangetypeBroadcastStyleBean;
        ...
        String str = "";
        if (SystemProperties.getBoolean(DebugProperties.KEY_ENABLE_OFFLINE_RANGETYPE, false)) {
            L.i(TAG, "load offline rangetype");
            try {
                FileInputStream fileInputStream = new FileInputStream(RootUtil.SD_CARD_NAVI_PATH + File.separator + OFFLINE_RANGETYPE_FILENAME);
                str = FileUtil.inputStream2String(fileInputStream);
            ...
            str = FileUtil.loadFromAssets(NGP_RANGETYPE_BROADCAST_JSON_DATA);
        }
        sNgpRangetypeBroadcastStyleBean = (NgpRangetypeBroadcastStyleBean) GsonUtil.fromJson(str, (Class<Object>) NgpRangetypeBroadcastStyleBean.class);
        BroadcastStyleBean broadcastStyleBean = sBroadcastStyleBean;
        if (broadcastStyleBean == null || TextUtils.isEmpty(broadcastStyleBean.getVersion()) || (ngpBroadcastStyleBean = sNgpBroadcastStyleBean) == null || TextUtils.isEmpty(ngpBroadcastStyleBean.getVersion()) || (ngpRangetypeBroadcastStyleBean = sNgpRangetypeBroadcastStyleBean) == null || TextUtils.isEmpty(ngpRangetypeBroadcastStyleBean.getVersion())) {
            L.Tag tag2 = TAG;
            L.w(tag2, "initAssetsData fatal. BroadcastStyleBean:" + sBroadcastStyleBean);
            return false;
        }
    ...
    private static void parseMaskList() {
        ...
        for (NgpRangetypeBroadcastPlaylistBean ngpRangetypeBroadcastPlaylistBean : sNgpRangetypeBroadcastStyleBean.getData()) {
            if (NGP_RANGETYPE_BROADCAST_DETAIL_TYPE.equals(ngpRangetypeBroadcastPlaylistBean.getType())) {
                sNgpRangetypeDetailList = ngpRangetypeBroadcastPlaylistBean.getPlayList();
            } else if (NGP_RANGETYPE_BROADCAST_SIMPLE_TYPE.equals(ngpRangetypeBroadcastPlaylistBean.getType())) {
                sNgpRangetypeSimpleList = ngpRangetypeBroadcastPlaylistBean.getPlayList();
            }
        }
    }
    ...
    public boolean filterTtsByMode(int i, int i2, int i3, String str) {
        if (!sIsInited) {
            L.i(TAG, "filterTtsByMode, not init yet");
            return true;
        }
        boolean filterNavTtsByMode = filterNavTtsByMode(i, i2, str);
        if (filterNavTtsByMode && CarFeatureManager.INSTANCE.isNgpTTSFilterBySoundTypeEnable()) {
            filterNavTtsByMode = filterNgpTtsByMode(i, i2);
        }
        return (filterNavTtsByMode && CarFeatureManager.INSTANCE.isNgpTtsFilterByRangetypeEnable()) ? filterNgpTtsByRangetype(i3) : filterNavTtsByMode;
    }
    ...
    private boolean filterNavTtsByMode(int i, int i2, String str) {
        if (isNeedSrMuteNaviTts(i, i2)) {
            ...
            return false;
        } else if (i < 10000 || (CarServiceManager.getInstance().isNGPOn() && TBTManager.getInstance().getCurrentStatus() != 2 && SRNaviManager.getInstance().isLaneEnabled())) {
    ...
    private boolean filterNgpTtsByRangetype(int i) {
        if (SRNaviManager.getInstance().isNgpState()) {
            int i2 = this.mNgpBroadCastType;
            if (1 == i2) {
                return filterNgpTtsByRangetype(i, sNgpRangetypeDetailList);
            }
            if (i2 == 0) {
                return filterNgpTtsByRangetype(i, sNgpRangetypeSimpleList);
            }
            return true;
        }
        return true;
    ...
    private boolean filterNgpTtsByRangetype(int i, List<NgpRangetypeBroadcastTypeBean> list) {
        for (NgpRangetypeBroadcastTypeBean ngpRangetypeBroadcastTypeBean : list) {
            if (ngpRangetypeBroadcastTypeBean.getRangeType().contains(Integer.valueOf(i))) {
                return true;
            }
        }
        return false;
