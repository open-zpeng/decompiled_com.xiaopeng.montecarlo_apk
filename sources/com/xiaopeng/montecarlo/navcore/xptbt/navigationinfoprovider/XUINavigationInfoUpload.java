...
public class XUINavigationInfoUpload implements INavigationInfoUploadStrategy, CarServiceManager.IXPCarStateListener {
    ...
    private static final String MSG_VERSION = "V1.0.12";
    ...
    public void updateSRInfo(boolean z) {
        ...
        JSONObject jSONObject2 = new JSONObject();
        try {
            jSONObject2.put("isSrMode", z);
            jSONObject.put("srinfo", jSONObject2);
            jSONObject.put("msgType", TBTManager.getInstance().getCurrentStatus());
            jSONObject.put("msgVersion", MSG_VERSION);
        } catch (JSONException e) {
            jSONObject = null;
            L.i(TAG, "updateSRInfo to json failed");
            e.printStackTrace();
        }
        if (jSONObject != null) {
            L.Tag tag = TAG;
            L.i(tag, "updateSRInfo data=" + LogHelper.appendLogLength(jSONObject.toString()));
            sendNavigationInfo(jSONObject.toString());
        }
    }
    ...
    public void updateSRTraffic(int i) {
        ...
        JSONObject jSONObject2 = new JSONObject();
        try {
            jSONObject2.put("trafficType", i);
            jSONObject.put("srTraffic", jSONObject2);
            jSONObject.put("msgType", TBTManager.getInstance().getCurrentStatus());
            jSONObject.put("msgVersion", MSG_VERSION);
        } catch (JSONException e) {
            jSONObject = null;
            L.i(TAG, "updateSRTraffic to json failed");
            e.printStackTrace();
        }
        if (jSONObject != null) {
            L.Tag tag = TAG;
            L.i(tag, "updateSRTraffic data=" + LogHelper.appendLogLength(jSONObject.toString()));
            sendNavigationInfo(jSONObject.toString());
        }
    }
    ...
    private void sendNavigationInfo(@NonNull String str) {
        if (!TextUtils.isEmpty(str)) {
            XUIServiceManager.getInstance().updateNavigationInfo(str.replace("\\/", "/"));
        } else {
    ...
    public void onIcmMapDisplayStateChange(boolean z) {
        if (z) {
            if (isNavi()) {
                ...
                updateSRInfo(TBTManager.getInstance().isLaneMode());
