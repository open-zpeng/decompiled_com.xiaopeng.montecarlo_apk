...
public class BLDataModelUtil {
    ...
    public static XPManeuverInfo toXPManeuverInfo(ManeuverInfo maneuverInfo) {
        ...
        if (maneuverInfo.dataBuf != null && maneuverInfo.dataBuf.buffer != null) {
            xPManeuverInfo.mDataBuf = (byte[]) maneuverInfo.dataBuf.buffer.clone();
        }
        return xPManeuverInfo;
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public static XPCameraInfo toXPCameraInfo(NaviCamera naviCamera) {
        int size;
        if (naviCamera == null) {
            return null;
        }
        XPCameraInfo xPCameraInfo = new XPCameraInfo();
        xPCameraInfo.mCoord2D = toXPCoord2DDouble(naviCamera.coord2D);
        xPCameraInfo.mCoord3D = toXPCoord3DDouble(naviCamera.coord3D);
        xPCameraInfo.mType = naviCamera.type;
        xPCameraInfo.mDistance = naviCamera.distance;
        if (naviCamera.speed != null && (size = naviCamera.speed.size()) > 0) {
            xPCameraInfo.mSpeed = new int[size];
            for (int i = 0; i < size; i++) {
                xPCameraInfo.mSpeed[i] = naviCamera.speed.get(i).shortValue();
            }
        }
        xPCameraInfo.mRoadClass = naviCamera.roadClass;
        return xPCameraInfo;
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public static XPCameraInfo toXPCameraInfo(NaviIntervalCamera naviIntervalCamera) {
        int size;
        if (naviIntervalCamera == null) {
            return null;
        }
        XPCameraInfo xPCameraInfo = new XPCameraInfo();
        xPCameraInfo.mCoord2D = toXPCoord2DDouble(naviIntervalCamera.coord2D);
        xPCameraInfo.mCoord3D = toXPCoord3DDouble(naviIntervalCamera.coord3D);
        xPCameraInfo.mType = naviIntervalCamera.type;
        xPCameraInfo.mDistance = naviIntervalCamera.distance;
        if (naviIntervalCamera.speed != null && (size = naviIntervalCamera.speed.size()) > 0) {
            xPCameraInfo.mSpeed = new int[size];
            for (int i = 0; i < size; i++) {
                xPCameraInfo.mSpeed[i] = naviIntervalCamera.speed.get(i).shortValue();
            }
        }
        return xPCameraInfo;
    }
    ...
    public static XPFacilityInfo toXPFacilityInfo(NaviFacility naviFacility) {
        ...
        xPFacilityInfo.mName = naviFacility.name;
    ...
    public static XPNaviCongestionInfo toXPNaviCongestionInfo(NaviCongestionInfo naviCongestionInfo) {
        ...
        if (currentNaviPathResult != null && currentNaviPathResult.getPath(naviCongestionInfo.getPathID()) != null) {
            xPNaviCongestionInfo.mPath = currentNaviPathResult.getPath(naviCongestionInfo.getPathID());
        }
        xPNaviCongestionInfo.mPathID = naviCongestionInfo.getPathID();
        xPNaviCongestionInfo.mTotalTimeOfSeconds = naviCongestionInfo.getTotalTimeOfSeconds();
        xPNaviCongestionInfo.mTotalRemainDist = naviCongestionInfo.getTotalRemainDist();
        xPNaviCongestionInfo.mUnobstructed = naviCongestionInfo.getUnobstructed();
        if (naviCongestionInfo.getCongestionInfos() != null && (size = naviCongestionInfo.getCongestionInfos().size()) > 0) {
            xPNaviCongestionInfo.mCongestionInfos = new XPNaviCongestionDetailInfo[size];
            for (int i = 0; i < size; i++) {
                xPNaviCongestionInfo.mCongestionInfos[i] = toXPNaviCongestionDetailInfo(naviCongestionInfo.getCongestionInfos().get(i));
            }
        }
