...
public class BLDataModelUtil {
    ...
    public static XPLaneInfo toXPLaneInfo(LaneInfo laneInfo, int i) {
        int size;
        int size2;
        if (laneInfo == null) {
            return null;
        ...
        if (laneInfo.backLane != null && (size2 = laneInfo.backLane.size()) > 0) {
            ...
            for (int i2 = 0; i2 < size2; i2++) {
                if (i2 != size2 - 1 && laneInfo.backLane.get(i2).intValue() == 22 && i != 255) {
                    xPLaneInfo.mBackLane[i2] = i;
                } else {
                    xPLaneInfo.mBackLane[i2] = laneInfo.backLane.get(i2).intValue();
                }
            }
        }
        if (laneInfo.frontLane != null && (size = laneInfo.frontLane.size()) > 0) {
            ...
            for (int i3 = 0; i3 < size; i3++) {
                if (i3 != size - 1 && laneInfo.frontLane.get(i3).intValue() == 22 && i != 255) {
                    xPLaneInfo.mFrontLane[i3] = i;
                } else {
                    xPLaneInfo.mFrontLane[i3] = laneInfo.frontLane.get(i3).intValue();
                }
            }
        }
    ...
    protected static XPRouteTrafficEventInfo toXPRouteTrafficEventInfo(RouteTrafficEventInfo routeTrafficEventInfo) {
    ...
    protected static XPTrafficEventInfo toXPTrafficEventInfo(TrafficEventInfo trafficEventInfo) {
        ...
        xPTrafficEventInfo.mLane = trafficEventInfo.f210lane;
    ...
    public static XPSoundInfo toXPSoundInfo(SoundInfo soundInfo) {
        ...
        xPSoundInfo.mRangeType = soundInfo.rangeType;
        return xPSoundInfo;
    }
