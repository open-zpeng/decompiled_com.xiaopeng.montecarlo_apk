...
public class GuideServiceWrapper {
    ...
    private static final String SDK_VERSION_650 = "9.650";
    ...
    private final ICruiseObserver mCruiseObserver = new ICruiseObserver() { // from class: com.xiaopeng.montecarlo.navcore.xptbt.GuideServiceWrapper.2
        ...
        public void onShowCruiseLaneInfo(LaneInfo laneInfo) {
            ...
            try {
                GuideServiceWrapper.this.mGuideManager.onUpdateCruiseLaneInfo(BLDataModelUtil.toXPLaneInfo(laneInfo, 255), true);
            } catch (Exception e) {
                NavCoreUtil.postCrashToast();
                e.printStackTrace();
    ...
    private final INaviObserver mNaviObserver = new INaviObserver() { // from class: com.xiaopeng.montecarlo.navcore.xptbt.GuideServiceWrapper.3
        boolean isAssistantActionNoGuide(int i) {
            return i == 0 || 5 == i || 6 == i || 19 == i || 20 == i || 23 == i || 24 == i || 25 == i || 32 == i || 33 == i || 34 == i || 35 == i || 36 == i || 37 == i || 50 == i;
        }

        boolean isMainActionNoGuide(int i) {
            return 8 == i || i == 0 || 9 == i || 14 == i;
        }
        ...
        public void onShowNaviLaneInfo(@Nullable LaneInfo laneInfo) {
            ...
            if (laneInfo == null) {
                L.i(GuideServiceWrapper.TAG, "onShowCruiseLaneInfo laneInfo == null");
                return;
            }
            int i = 255;
            if (isNeedFillEmptyLane(laneInfo)) {
                int[] mainActionAndAssistantActionByPathInfo = getMainActionAndAssistantActionByPathInfo(laneInfo.segmentIdx, laneInfo.linkIdx);
                i = getLaneByMainActionAndAssistantAction(mainActionAndAssistantActionByPathInfo[0], mainActionAndAssistantActionByPathInfo[1]);
            }
            try {
                GuideServiceWrapper.this.mGuideManager.onUpdateNaviLaneInfo(BLDataModelUtil.toXPLaneInfo(laneInfo, i), true);
            } catch (Exception e) {
                NavCoreUtil.postCrashToast();
                e.printStackTrace();
                L.Tag tag = GuideServiceWrapper.TAG;
        ...
        private int[] getMainActionAndAssistantActionByPathInfo(int i, int i2) {
            PathInfo variantPath;
            SegmentInfo segmentInfo;
            LinkInfo linkInfo;
            int[] iArr = {0, 0};
            IVariantPath variantPath2 = TBTManager.getInstance().getVariantPath(TBTManager.getInstance().getCurrentNaviId(), TBTManager.getInstance().getCurrentNaviPathIndex());
            if (variantPath2 != null && (variantPath = ((XPVariantPath) variantPath2).getVariantPath()) != null && (segmentInfo = variantPath.getSegmentInfo(i)) != null && (linkInfo = segmentInfo.getLinkInfo(i2)) != null) {
                iArr[0] = linkInfo.getMainAction();
                iArr[1] = linkInfo.getAssistantAction();
            }
            return iArr;
        }

        private int getLaneByMainActionAndAssistantAction(int i, int i2) {
            if (L.ENABLE) {
                L.Tag tag = GuideServiceWrapper.TAG;
                L.d(tag, "getLaneByMainActionAndAssistantAction mainAction = " + i + ", assistantAction = " + i2);
            }
            return (isMainActionNoGuide(i) && isAssistantActionNoGuide(i2)) ? 0 : 255;
        }

        boolean isNeedFillEmptyLane(LaneInfo laneInfo) {
            IVariantPath variantPath;
            XPDrivePathAccessor drivePathAccessor;
            if (ServiceMgr.getVersion().contains(GuideServiceWrapper.SDK_VERSION_650) || (variantPath = TBTManager.getInstance().getVariantPath(TBTManager.getInstance().getCurrentNaviId(), TBTManager.getInstance().getCurrentNaviPathIndex())) == null || (drivePathAccessor = variantPath.getDrivePathAccessor()) == null) {
                return false;
            }
            XPBaseSegmentAccessor segmentAccessor = drivePathAccessor.getSegmentAccessor(laneInfo.segmentIdx);
            if (segmentAccessor == null) {
                drivePathAccessor.recycle();
                return false;
            }
            XPBaseLinkAccessor linkAccessor = segmentAccessor.getLinkAccessor(laneInfo.linkIdx);
            ...
            if (L.ENABLE) {
                L.Tag tag = GuideServiceWrapper.TAG;
                L.d(tag, "isNeedFillEmptyLane roadClass = " + roadClass);
            }
            return roadClass == 0 || roadClass == 6;
        }
        ...
        public void onNaviStop(long j, int i) {
            ...
            try {
                GuideServiceWrapper.this.mGuideManager.postNaviStop(i, true);
            } catch (Exception e) {
                NavCoreUtil.postCrashToast();
                e.printStackTrace();
                L.Tag tag2 = GuideServiceWrapper.TAG;
                    ...
                    if (routeOption != null) {
                        if (14 == routeOption.getRouteType()) {
                            ...
                            if (j <= 1000) {
                                ...
                                if (routeOption != null) {
                                    L.i(GuideServiceWrapper.TAG, "onReroute rerouteOption.delete");
                                    routeOption.delete();
                                    return;
                                }
                                return;
