...
public class GuideServiceWrapper {
    ...
    private final INaviObserver mNaviObserver = new INaviObserver() { // from class: com.xiaopeng.montecarlo.navcore.xptbt.GuideServiceWrapper.3
        ...
        public void onShowNaviCameraExt(ArrayList<NaviCameraExt> arrayList) {
            GuideServiceWrapper.this.handleCamera(arrayList);
        }
        ...
        public void onDriveReport(DriveReport driveReport) {
            GuideServiceWrapper.this.mGuideManager.onDriveReport(driveReport);
        }
    ...
    public void handleCamera(ArrayList<NaviCameraExt> arrayList) {
        if (arrayList == null) {
            return;
        }
        try {
            ...
            Iterator<NaviCameraExt> it = arrayList.iterator();
            while (it.hasNext()) {
                NaviCameraExt next = it.next();
                ArrayList<NaviSubCameraExt> arrayList4 = next.subCameras;
                if (arrayList4 != null) {
                    int size = arrayList4.size();
                    boolean z = false;
                    for (int i = 0; i < size; i++) {
                        NaviSubCameraExt naviSubCameraExt = arrayList4.get(i);
                        if (naviSubCameraExt != null) {
                            if (NavCoreUtil.isNaviIntervalCamera(naviSubCameraExt.subType)) {
                                if (naviSubCameraExt.speed != null && naviSubCameraExt.speed.size() != 0 && naviSubCameraExt.speed.get(0).shortValue() != 0 && naviSubCameraExt.speed.get(0).shortValue() != 255) {
                                    arrayList2.add(BLDataModelUtil.toXPCameraInfo(next, naviSubCameraExt.subType, naviSubCameraExt.speed, z));
                                    if (!z) {
                                        z = true;
                                    }
                                }
                            } else {
                                arrayList3.add(BLDataModelUtil.toXPCameraInfo(next, naviSubCameraExt.subType, naviSubCameraExt.speed, z));
                                if (z) {
                                }
                                z = true;
                            }
            ...
            this.mGuideManager.onShowNaviIntervalCamera(arrayList2);
            this.mGuideManager.onShowNaviCamera(arrayList3);
        } catch (Exception e) {
            NavCoreUtil.postCrashToast();
            e.printStackTrace();
            L.e(TAG, "handleCamera error:" + e.getMessage());
        }
    }
    ...
    public boolean initGuide() {
        if (this.mGuideService == null) {
            this.mGuideService = (GuideService) ServiceMgr.getServiceMgrInstance().getBLService(16);
            if (this.mGuideService == null) {
    ...
    public void setNaviPath(INaviPath iNaviPath, int i) {
        if (checkGuideServiceInitialized() && iNaviPath != null) {
            ...
            XPNaviPath xPNaviPath = (XPNaviPath) iNaviPath;
            xPNaviPath.getNaviPath().setMainIdx((long) i);
            this.mGuideService.setNaviPath(xPNaviPath.getNaviPath());
