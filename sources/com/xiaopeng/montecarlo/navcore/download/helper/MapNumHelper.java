...
public class MapNumHelper {
    ...
    public void initInternetNum() {
        boolean isMapNumOutOfDate = isMapNumOutOfDate(DataSetHelper.GlobalSet.getLong(DataSetHelper.GlobalSet.INTERNET_MAP_NUM_LAST_UPDATE_TIME, -1L));
        MapNum mapNum = new MapNum();
        mapNum.strKey = ContextUtils.getString(R.string.setting_unity_internet_map_num_key);
        if (isMapNumOutOfDate) {
            DataSetHelper.GlobalSet.set(DataSetHelper.GlobalSet.INTERNET_MAP_NUM_LAST_UPDATE_TIME, Calendar.getInstance().getTimeInMillis());
            updateInternetMapNum(mapNum);
        }
    }

    private void updateInternetMapNum(MapNum mapNum) {
        MapDataManager.getInstance().requestMapNum(mapNum, new IMapNumObserver() { // from class: com.xiaopeng.montecarlo.navcore.download.helper.-$$Lambda$MapNumHelper$LZC7t4xXHn8yAza57e_YXEuOKB0
            @Override // com.autonavi.gbl.data.observer.IMapNumObserver
            public final void onRequestMapNum(int i, MapNum mapNum2) {
                MapNumHelper.lambda$updateInternetMapNum$2(i, mapNum2);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static /* synthetic */ void lambda$updateInternetMapNum$2(int i, MapNum mapNum) {
        if (mapNum != null) {
            if (L.ENABLE) {
                L.Tag tag = TAG;
                L.d(tag, "IMapNumObserver opErrCode=" + i + ",mapNumContent=" + mapNum.strContent + ",mapNumKey=" + mapNum.strKey + ",mapNumVersion=" + mapNum.strVersion);
            }
            DataSetHelper.GlobalSet.set(DataSetHelper.GlobalSet.INTERNET_MAP_NUM_DATA, GsonUtil.toJson(mapNum));
        }
    }
    ...
    private MapNum getLocalInternetMapNum() {
        String string = DataSetHelper.GlobalSet.getString(DataSetHelper.GlobalSet.INTERNET_MAP_NUM_DATA, "");
        if (TextUtils.isEmpty(string)) {
            return null;
        }
        try {
