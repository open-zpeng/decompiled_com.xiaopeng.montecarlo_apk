...
public class CityDataBean {
    ...
    public long getLocalDataSize() {
        if (isFinish()) {
            return getDataFullSize();
        }
        if (this.mPercent != 0.0f) {
            Log.i("CityDataBean", "tMapData,getLocalDataSize urcode:" + this.mUrcode + ",adCode:" + this.mCityDownLoadItem.adcode + " ,taskCode:" + getTaskState() + "-MapDataSer mpercent:" + this.mPercent + ",mCityDownLoadItem.percent:" + this.mCityDownLoadItem.percent + "，cityName" + getCityTitle());
            return (((float) this.mCityDownLoadItem.nFullZipSize.longValue()) * this.mPercent) / 100.0f;
        }
        return 0L;
    }
}
