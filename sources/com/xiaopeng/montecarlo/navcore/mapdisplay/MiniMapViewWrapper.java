...
public class MiniMapViewWrapper extends MapViewWrapper {
    ...
    protected void creatMapSurfaceView(@Nullable Application application, int i, int i2, int i3, MapViewWrapper.IXPMapSurfaceListener iXPMapSurfaceListener) {
        if (this.mMapView == null) {
            ...
            deviceAttribute.maxVideoMemory = 1500;
            ...
            ServiceMgr.getServiceMgrInstance().setUiLooper(this.mDeviceId, WorkThreadUtil.getInstance().getMiniBizControlLooper());
    ...
    public int getDefaultMapViewTop() {
        float mapHeight;
        float miniMapCarUpRatio;
        if (checkMapViewValid()) {
            if (isLaneMode()) {
                mapHeight = getMapHeight();
                miniMapCarUpRatio = 0.75f;
            } else {
                mapHeight = getMapHeight();
                miniMapCarUpRatio = CarFeatureManager.INSTANCE.getMiniMapCarUpRatio();
            }
            int i = (int) (mapHeight * miniMapCarUpRatio);
            if (L.ENABLE) {
                L.d(TAG, L.formatWithEngine("getDefaultMapViewTop: " + i, getMainEngineID()));
            }
            return i;
        }
        return 0;
    ...
    protected MapStyleParam formatMapStyleParam(int i, int i2) {
        ...
        if (i2 == 1) {
            ...
            mapStyleParam.style = z ? MapModelConstants.MAP_STYLE_LANE_EXTEND_DAY : MapModelConstants.MAP_STYLE_LANE_EXTEND_NIGHT;
        } else {
    ...
    public void detachSurfaceToDevice() {
        this.mMapDevice.detachSurfaceFromDevice();
    }
}
