...
import com.xiaopeng.montecarlo.navcore.sr.LaneResourceManager;
...
import com.xiaopeng.montecarlo.root.util.ioutil.EncryptZipUtil;
...
public class XPOfflineDataService {
    ...
    public boolean initChargeData() {
        ...
        if (CarFeatureManager.INSTANCE.isOfflineIncrementEnabled()) {
            return initChargeDataPatch();
        }
        return initChargeDataWithoutPatch();
    }

    private boolean checkDataCorrect() {
        if (0 == CPDataHelper.listCount(CPInfo.class)) {
            return CPDataHelper.clearData();
        }
        if (0 == CPDataHelper.listCount(Level500CellInfo.class)) {
            return CPDataHelper.replaceData(CPDataHelper.generateCellInfoList(CPDataHelper.listAll(CPInfo.class)), Level500CellInfo.class);
        }
        return true;
    }

    public boolean initChargeDataPatch() {
        String str;
        boolean parseIncrementData;
        if (checkDataCorrect()) {
            long chargeDataUpdateTime = SettingWrapper.getChargeDataUpdateTime();
            if (chargeDataUpdateTime > 0 && TimeUtil.isSameDay(chargeDataUpdateTime, System.currentTimeMillis())) {
                L.i(TAG, "initChargeData has update today ,update time:" + chargeDataUpdateTime);
                return true;
            }
            String chargeDbVersion = SettingWrapper.getChargeDbVersion();
            if (TextUtils.isEmpty(chargeDbVersion) || chargeDbVersion.length() < 14) {
                L.e(TAG, "initChargeData queryChargeOfflineDataPatch currentVersionError:" + chargeDbVersion);
                chargeDbVersion = DEFAULT_VERSION_ID;
            }
            NaviOfflineDataVersionResponse queryChargeOfflineDataPatch = this.mChargeDataProvider.queryChargeOfflineDataPatch(chargeDbVersion);
            if (queryChargeOfflineDataPatch == null) {
                if (L.ENABLE) {
                    L.d(TAG, "initChargeData chargeVersionResponse is null");
                }
                return false;
            }
            if (L.ENABLE) {
                L.d(TAG, "11 initChargeData currentVersion:" + chargeDbVersion + ", newVersion:" + queryChargeOfflineDataPatch.getLatestDataVersion());
            }
            if (chargeDbVersion.compareTo(queryChargeOfflineDataPatch.getLatestDataVersion()) >= 0 || TextUtils.isEmpty(queryChargeOfflineDataPatch.getDataUrl())) {
                SettingWrapper.setChargeDataUpdateTime(System.currentTimeMillis());
                return true;
            }
            if (queryChargeOfflineDataPatch.isNoDataPatch()) {
                parseIncrementData = true;
            } else {
                String[] split = queryChargeOfflineDataPatch.getDataUrl().split("/");
                if (split.length == 0) {
                    L.e(TAG, "urlStrArr parse error getDataUrl:" + queryChargeOfflineDataPatch.getDataUrl());
                    return false;
                }
                String str2 = split[split.length - 1];
                if (!HttpDownloadUtil.downloadFromUrl(queryChargeOfflineDataPatch.getDataUrl(), RootUtil.getDataPath() + RootUtil.SD_CARD_DIR_CHARGE_DATA_PATH, str2)) {
                    if (L.ENABLE) {
                        L.d(TAG, "initChargeData download error");
                    }
                    return false;
                }
                String str3 = RootUtil.getDataPath() + RootUtil.SD_CARD_DIR_CHARGE_DATA_PATH + File.separator + str2;
                File file = new File(str3);
                L.d(TAG, "initChargeData unzip zipPath = " + str3);
                if (file.exists()) {
                    L.d(TAG, "initChargeData unzip uncompressEncodedZip  true ");
                    str = EncryptZipUtil.uncompressEncodedZip(file, file.getParent());
                } else {
                    str = "";
                }
                if (L.ENABLE) {
                    L.d(TAG, "initChargeData unzip result = " + str);
                }
                if (TextUtils.isEmpty(str)) {
                    if (L.ENABLE) {
                        L.d(TAG, "initChargeData unzip error");
                    }
                    return false;
                }
                parseIncrementData = CPDataHelper.parseIncrementData(str, queryChargeOfflineDataPatch.isNormalPatch());
            }
            if (parseIncrementData) {
                SettingWrapper.setChargeDataUpdateTime(System.currentTimeMillis());
                SettingWrapper.setChargeDbVersion(queryChargeOfflineDataPatch.getLatestDataVersion());
                if (L.ENABLE) {
                    L.d(TAG, "initChargeData finished success");
                }
                return true;
            }
            return false;
    ...
    */
    public boolean initChargeDataWithoutPatch() {
        /*
            Method dump skipped, instructions count: 354
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: com.xiaopeng.montecarlo.navcore.search.offlinedata.XPOfflineDataService.initChargeDataWithoutPatch():boolean");
    }
    ...
    private XUIResourceResponse querySRCarModel() {
        ...
        hashMap.put("car_subtype", "" + LaneResourceManager.SCISSORS_GATE);
        hashMap.put("car_color", "" + LaneResourceManager.BODY_COLOR);
