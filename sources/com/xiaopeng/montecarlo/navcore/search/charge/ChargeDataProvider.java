...
import com.xiaopeng.montecarlo.navcore.httpclient.interfaces.INaviSearchV3Service;
...
public class ChargeDataProvider {
    ...
    private final INaviSearchV3Service mNaviSearchV3Service = (INaviSearchV3Service) HttpClientManager.getInstance().getService(INaviSearchV3Service.class);
    ...
    public NaviOfflineDataVersionResponse queryChargeOfflineDataPatch(String str) {
        return this.mNaviSearchV3Service.downloadPatch(new NaviDownloadRequest().setCategory(XPNaviSearchParam.PARAM_CATEGORY_VALUE_CHARGE).setCurrentDataVersion(str));
    }
    ...
    private XPPoiInfo generalChargeInfo2XPPoiInfo(NaviPoiInfo naviPoiInfo) {
        ...
        if (chargeStation != null) {
            ...
            xPDeepChargingArr[0].setNumUltra(chargeStation.getUltraNum());
            if (chargeStation.getUltraNum() > 0) {
                xPDeepChargingArr[0].setNumUltraFree(chargeStation.getUltraFreeNum());
            } else {
                xPDeepChargingArr[0].setNumUltraFree(-1);
            }
            xPDeepChargingArr[0].setNumFast(chargeStation.getFastNum() - chargeStation.getUltraNum());
            if (chargeStation.getFastNum() > 0) {
                xPDeepChargingArr[0].setNumFastFree(chargeStation.getFastFreeNum() - chargeStation.getUltraFreeNum());
            } else {
