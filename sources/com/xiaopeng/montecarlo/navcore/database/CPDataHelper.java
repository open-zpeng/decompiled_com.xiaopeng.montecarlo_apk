...
public class CPDataHelper {
    ...
    public static boolean replaceAll500CellData(final List<Level500CellInfo> list) {
        final DaoSession daoSession = DatabaseManager.getInstance().getDaoSession();
        if (daoSession != null && !CollectionUtils.isEmpty(list)) {
            try {
                Callable<Boolean> callable = new Callable<Boolean>() { // from class: com.xiaopeng.montecarlo.navcore.database.CPDataHelper.3
                    /* JADX WARN: Can't rename method to resolve collision */
                    @Override // java.util.concurrent.Callable
                    public Boolean call() throws Exception {
                        DaoSession.this.deleteAll(Level500CellInfo.class);
                        Iterator it = list.iterator();
                        while (it.hasNext()) {
                            if (DaoSession.this.insert((Level500CellInfo) it.next()) < 0) {
                                throw new Exception();
                            }
                        }
                        return true;
                ...
                if (L.ENABLE) {
                    L.d(TAG, "replaceAll500CellData begin insert");
                }
                boolean booleanValue = ((Boolean) daoSession.callInTx(callable)).booleanValue();
                if (L.ENABLE) {
                    L.Tag tag = TAG;
                    L.d(tag, "replaceAll500CellData end insert result:" + booleanValue);
                }
            } catch (Exception e) {
                e.getStackTrace();
            }
    ...
    public static boolean updateCPInfoList(final List<CPInfo> list, final List<CPInfo> list2, final List<CPInfo> list3) {
        final DaoSession daoSession = DatabaseManager.getInstance().getDaoSession();
        boolean z = false;
        try {
            Callable<Boolean> callable = new Callable<Boolean>() { // from class: com.xiaopeng.montecarlo.navcore.database.CPDataHelper.4
                /* JADX WARN: Can't rename method to resolve collision */
                @Override // java.util.concurrent.Callable
                public Boolean call() throws Exception {
                    List list4 = list;
                    if (list4 != null) {
                        ...
                        while (it.hasNext()) {
                            if (daoSession.insert((CPInfo) it.next()) < 0) {
                                throw new Exception();
                            }
                        }
                        L.Tag tag = CPDataHelper.TAG;
                        L.i(tag, "updateCPInfoList add size:" + list.size());
                    }
                    List<CPInfo> list5 = list2;
                    if (list5 != null) {
                        for (CPInfo cPInfo : list5) {
                            List list6 = daoSession.queryBuilder(CPInfo.class).where(CPInfoDao.Properties.StationId.in(cPInfo.getStationId()), new WhereCondition[0]).list();
                            if (list6 == null || list6.size() != 1) {
                                L.Tag tag2 = CPDataHelper.TAG;
                                L.e(tag2, "no local station found ,update fail!:" + cPInfo.getStationName());
                                throw new Exception();
                            }
                            cPInfo.setId(((CPInfo) list6.get(0)).getId());
                            daoSession.update(cPInfo);
                        }
                        L.Tag tag3 = CPDataHelper.TAG;
                        L.i(tag3, "updateCPInfoList update size:" + list2.size());
                    }
                    if (list3 != null) {
                        ArrayList arrayList = new ArrayList();
                        for (CPInfo cPInfo2 : list3) {
                            arrayList.add(cPInfo2.getStationId());
                        }
                        List<CPInfo> list7 = daoSession.queryBuilder(CPInfo.class).where(CPInfoDao.Properties.StationId.in(arrayList), new WhereCondition[0]).list();
                        if (list7 == null || list7.size() != list3.size()) {
                            L.Tag tag4 = CPDataHelper.TAG;
                            StringBuilder sb = new StringBuilder();
                            sb.append("delete fail size not equal local size:");
                            sb.append(list7 != null ? list7.size() : 0);
                            sb.append(" delete size:");
                            sb.append(list3.size());
                            L.e(tag4, sb.toString());
                            throw new Exception();
                        }
                        for (CPInfo cPInfo3 : list7) {
                            daoSession.delete(cPInfo3);
                        }
                        L.Tag tag5 = CPDataHelper.TAG;
                        L.i(tag5, "updateCPInfoList delete size:" + list3.size());
                    }
                    return true;
                }
            };
            if (L.ENABLE) {
                L.d(TAG, "updateCPInfoList begin insert");
            }
            z = ((Boolean) daoSession.callInTx(callable)).booleanValue();
            if (L.ENABLE) {
                L.Tag tag = TAG;
                L.d(tag, "updateCPInfoList end insert success:" + z);
            }
        } catch (Exception e) {
            e.getStackTrace();
            L.Tag tag2 = TAG;
            L.e(tag2, L.printStackTrace(tag2, e));
        }
        return z;
    }
    ...
    */
    ...
    /*
        ...
        To view partially-correct add '--show-bad-code' argument
    */
    public static boolean parseIncrementData(java.lang.String r9, boolean r10) {
        /*
            Method dump skipped, instructions count: 486
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: com.xiaopeng.montecarlo.navcore.database.CPDataHelper.parseIncrementData(java.lang.String, boolean):boolean");
    }
