...
class NaviPdPresenter implements BaseCallBack<PowerDisRequestParam, PowerDisResultData>, INaviOnLinePdContract.Presenter, PowerDisTimerWorker.IPowerDisOnTimerListener, IXPNetworkListener {
    ...
    public void onTimerRefreshOnlineK() {
        ...
        boolean z = false;
        if (currentNaviRouteParams != null && currentNaviRouteParams.mViaInfos != null) {
            int size = currentNaviRouteParams.mViaInfos.size();
            for (int i = 0; i < size; i++) {
                XPViaPoint xPViaPoint = currentNaviRouteParams.mViaInfos.get(i);
    ...
    private void refreshPanel(boolean z) {
        ...
        double k = this.mOnlineKHolder.getK() * getRouteRemain();
        double d = carRemainDistance - k;
        if (d >= 1000.0d) {
            if (this.mIsNetworkConnected && z) {
                ...
                if (!z2) {
                    if (L.ENABLE) {
                        L.Tag tag2 = TAG;
                        L.d(tag2, "refreshOnlineK--startSegIndex：" + this.mStartSegIndex + " startLinkIndex:" + this.mStartLinkIndex + " disToTarget:" + this.mDisToTarget + "   pathId:" + this.mPathId);
                    }
            ...
            if (this.mIsKValid) {
                int i = carRemainDistance - ((int) k);
                if (L.ENABLE) {
                    L.Tag tag3 = TAG;
                    L.d(tag3, "refreshPanel UseValidKey  >>>remainDis:" + i + "  carRemain:" + carRemainDistance + "  routeRemain:" + k + "  k:" + this.mOnlineKHolder.getK() + "  mPathId:" + this.mPathId);
                }
                refreshPanel(i, true);
                return;
            }
            if (L.ENABLE) {
                L.Tag tag4 = TAG;
                L.d(tag4, "refreshPanel  key is not valid >>>> try to refreshWithOfflineK--startSegIndex：" + this.mStartSegIndex + " startLinkIndex:" + this.mStartLinkIndex + " disToTarget:" + this.mDisToTarget + "   pathId:" + this.mPathId);
            }
        ...
        if (L.ENABLE) {
            L.Tag tag5 = TAG;
            L.d(tag5, "refreshPanel onTimerRefreshPanel(0) for reason( cannot ReachDes)   pathId:" + this.mPathId + "   carRemain:" + CarServiceManager.getInstance().getCarRemainDistance() + "  mDisToTarget:" + this.mDisToTarget);
        }
        refreshPanel(0, true);
    }
    ...
    private void refreshPanel(int i, boolean z) {
        L.Tag tag = TAG;
        L.i(tag, "refreshPanel remainDis:" + i + " isOnline:" + z + " mDisToTarget:" + this.mDisToTarget + "   pathId:" + this.mPathId);
