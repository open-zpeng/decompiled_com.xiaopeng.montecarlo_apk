...
public class XpNgpPanelView extends XRelativeLayout implements View.OnClickListener, XCountDownTimer.ITimerCallback {
    ...
    public void updateNgpOperateBtnStatus(boolean z) {
        this.mLcTipsContentOperate.setVisibility((!z || TextUtils.isEmpty((String) this.mLcTipsContentOperate.getText())) ? 8 : 0);
        refreshLcTipsLayoutTargetWidth();
    }
    ...
    public void updateNgpLcTips(int i, int i2, boolean z, Drawable drawable, String str) {
        ...
        if (L.ENABLE) {
            L.Tag tag = TAG;
            L.d(tag, "updateNgpLcTipsAndOddStatus panelStatus = " + i + ",lcTipsStatus = " + i2 + " lcTipsText = " + str);
        }
    ...
    private void updateAutoLayout(String str) {
        stopArrowAnim();
        if (TextUtils.isEmpty(str)) {
            this.mLcTipsDriveLevelContent.setText(R.string.srnavi_ngp_panel_enter_available_area);
        } else {
    ...
    private void updateFunctionIconLayout() {
        stopArrowAnim();
        stopAnimSet();
        this.mLcTipsSingleIcon.setVisibility(0);
        this.mLcTipsSingleLoading.setVisibility(8);
    ...
    public void onClick(View view) {
        if (this.mPanelListener != null) {
            ...
            } else if (this.mLcTipsContentOperate.getVisibility() == 0) {
