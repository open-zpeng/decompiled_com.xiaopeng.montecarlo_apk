...
public class PoiCardChargeStatusContainer extends XRelativeLayout implements BasePoiCardViewContract {
    ...
    public void renderChargeView(final XPDeepCharging xPDeepCharging, final int i, final boolean z) {
        ...
        post(new Runnable() { // from class: com.xiaopeng.montecarlo.views.poicard.PoiCardChargeStatusContainer.1
            ...
            public void run() {
                ...
                int viewWidth;
                ChargeView chargeView2;
                PoiCardChargeStatusContainer.this.mChargeContainer.removeAllViews();
                if (xPDeepCharging.getNumUltraFree() > 0 || xPDeepCharging.getNumUltra() > 0 || xPDeepCharging.getNumFastFree() > 0 || xPDeepCharging.getNumFast() > 0 || xPDeepCharging.getNumSlowFree() > 0 || xPDeepCharging.getNumSlow() > 0) {
                    ChargeView chargeView3 = null;
                    if (xPDeepCharging.getNumUltraFree() > 0 || xPDeepCharging.getNumUltra() > 0) {
                        ...
                        chargeView.initChargeData(0, xPDeepCharging.getNumUltraFree(), xPDeepCharging.getNumUltra(), z);
                        viewWidth = RootUtil.getViewWidth(chargeView) + 0;
                    ...
                    if (xPDeepCharging.getNumFastFree() > 0 || xPDeepCharging.getNumFast() > 0) {
                        ...
                        chargeView2.initChargeData(1, xPDeepCharging.getNumFastFree(), xPDeepCharging.getNumFast(), z);
                        viewWidth += RootUtil.getViewWidth(chargeView2);
                    ...
                    if (xPDeepCharging.getNumSlowFree() > 0 || xPDeepCharging.getNumSlow() > 0) {
                        chargeView3 = new ChargeView(PoiCardChargeStatusContainer.this.getContext());
                        chargeView3.initChargeData(2, xPDeepCharging.getNumSlowFree(), xPDeepCharging.getNumSlow(), z);
                        viewWidth += RootUtil.getViewWidth(chargeView3);
                        PoiCardChargeStatusContainer.this.mChargeContainer.addView(chargeView3);
                    }
                    if (viewWidth > i) {
                    ...
                    PoiCardChargeStatusContainer.this.calculateChargeViewPosition(chargeView, chargeView2, chargeView3);
                }
            }
        });
    }
    ...
    public void calculateChargeViewPosition(ChargeView chargeView, ChargeView chargeView2, ChargeView chargeView3) {
        if (this.mChargeContainer != null) {
            setChargeViewLayoutParams(chargeView, 0);
            setChargeViewLayoutParams(chargeView2, 1);
            setChargeViewLayoutParams(chargeView3, 2);
        }
    }

    private void setChargeViewLayoutParams(ChargeView chargeView, int i) {
        ...
        if (this.mChargeContainer.getOrientation() == 1 && i == 0) {
    ...
    public void renderChargeView(XPDeepCharging xPDeepCharging) {
        ...
        boolean z = xPDeepCharging.getNumUltraFree() > 0 || xPDeepCharging.getNumUltra() > 0 || xPDeepCharging.getNumFastFree() > 0 || xPDeepCharging.getNumFast() > 0 || xPDeepCharging.getNumSlowFree() > 0 || xPDeepCharging.getNumSlow() > 0;
        setVisibility(z ? 0 : 8);
        if (z) {
