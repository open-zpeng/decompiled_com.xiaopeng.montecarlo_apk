...
public class VolumePanelView implements PopupWindow.OnDismissListener, SeekBar.OnSeekBarChangeListener {
    ...
    public void toggleMute(boolean z) {
        ...
        if (Utils.isPortrait()) {
            ...
            this.mMuteBtn.setImageLevel(getVolumeLevel(z, volume));
            return;
        }
    ...
    private int getVolumeLevel(boolean z, int i) {
        if (z) {
            return 0;
        }
        float maxVolume = i / AudioUtil.getInstance().getMaxVolume();
        if (maxVolume > 0.6f) {
            return 3;
        }
        return maxVolume > 0.2f ? 2 : 1;
    }
