...
public class SearchPresenter extends BasePresenter implements ISearchContract.Presenter, BaseCallBack<SearchRequestParam, SearchResultData> {
    ...
    public void goToPoi(XPPoiInfo xPPoiInfo, List<XPPoiInfo> list) {
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            ListIterator<XPPoiInfo> listIterator = list.listIterator();
            XPPoiInfo next = listIterator.next();
            arrayList.add(new XPViaPoint(next));
            while (listIterator.hasNext() && arrayList.size() < 3) {
                XPPoiInfo next2 = listIterator.next();
                if (next.getPoiId().equals(next2.getPoiId())) {
                    listIterator.remove();
                } else {
                    arrayList.add(new XPViaPoint(next2));
                    next = next2;
                }
            }
        }
    ...
    private void switchCity(XPPoiInfo xPPoiInfo, RectFloat rectFloat) {
        ...
        PointD lonLatToGeoPoint = this.mLogicView.getMapView().lonLatToGeoPoint(new XPCoordinate2DDouble(rectFloat.left, rectFloat.top));
        PointD lonLatToGeoPoint2 = this.mLogicView.getMapView().lonLatToGeoPoint(new XPCoordinate2DDouble(rectFloat.right, rectFloat.bottom));
        if (lonLatToGeoPoint == null || lonLatToGeoPoint2 == null) {
            return;
        }
        float mapZoom = this.mLogicView.getMapView().getMapZoom((int) lonLatToGeoPoint.x, (int) lonLatToGeoPoint.y, (int) lonLatToGeoPoint2.x, (int) lonLatToGeoPoint2.y);
