...
public class CarLocIconHelper implements ICarLocIconHelper, CarServiceManager.IXPCarStateListener, ISRInfoChangedListener {
    ...
    public void updateCarLocIconDynamicOnRun() {
        ...
        if (i <= 0) {
            i2 = -1;
        } else if (SRDataUtil.isNgpDriveMode(this.mScuDriveMode)) {
        ...
        if (i2 < 0 || i5 < 0 || i <= 0) {
            if (SRDataUtil.isNgpDriveMode(this.mScuDriveMode)) {
    ...
    private void updateCarLocTextures() {
        ...
        MarkUtils.addOverlayTexturesAsync(mapViewWrapper, mapViewWrapper.getMainEngineID(), this.mCarLocList);
    }
