...
public class MapDataProvider extends BaseDataProvider<MapSceneResultData, MapSceneRequestParam, BaseCallBack> {
    ...
    */
    ...
    private List<XPPoiInfo> filterInScreenCP(List<XPPoiInfo> list) {
        ...
        ArrayList arrayList = new ArrayList();
        XPPoiInfo expandChargeDecoratorInfo = this.mCallBack.getExpandChargeDecoratorInfo();
        for (XPPoiInfo xPPoiInfo : list) {
            Point lonLatToPoint = this.mCallBack.lonLatToPoint(new XPCoordinate2DDouble(xPPoiInfo));
            if (lonLatToPoint != null) {
                if (expandChargeDecoratorInfo != null && expandChargeDecoratorInfo.getPoiId().equalsIgnoreCase(xPPoiInfo.getPoiId())) {
                    arrayList.add(xPPoiInfo);
                } else if (lonLatToPoint.x >= 0 && lonLatToPoint.x <= mapStatus.getWidth() && lonLatToPoint.y >= 0 && lonLatToPoint.y <= mapStatus.getHeight() && arrayList.size() < 50) {
                    arrayList.add(xPPoiInfo);
                }
            }
        }
