...
public class MapPrepareLayerStyle implements IPrepareLayerStyle, ActivityLifecycleHelper.ActivityLifecycleListener {
    ...
    */
    ...
    private int createGuideCongestionMarkerId(GuideCongestionLayerItem guideCongestionLayerItem) {
        ...
        String mTimeInfo = guideCongestionLayerItem.getMTimeInfo();
        guideCongestionLayerItem.getMTotalTimeOfSeconds();
        guideCongestionLayerItem.getMTotalRemainDist();
        ...
        int congestionLabelBgResource = ResUtil.getCongestionLabelBgResource(mDirectionStyle, (mCongestionStatus == 0 || 5 == mCongestionStatus) ? 1 : mCongestionStatus, ThemeWatcherUtil.isDayMode());
        if (congestionLabelBgResource != 0) {
            ...
            XPNaviCongestionDetailInfo naviCongestionDetailInfo = NaviUtil.getNaviCongestionDetailInfo(TBTManager.getInstance().getNaviCongestionInfo(), mCongestionStatus);
            if (naviCongestionDetailInfo != null) {
                int i = naviCongestionDetailInfo.mRemainDist;
                if (i > 0) {
                    strArr = NaviUtil.getDistanceArray(i);
                }
                List<String> timeListString = naviCongestionDetailInfo.mTimeOfSeconds > 0 ? NaviUtil.getTimeListString(naviCongestionDetailInfo.mTimeOfSeconds) : null;
                if (timeListString == null || CollectionUtils.isEmpty(timeListString)) {
                    timeListString = NaviUtil.getTimeListString(mTimeInfo);
                }
                L.Tag tag = TAG;
                StringBuilder sb = new StringBuilder();
                sb.append("getBizPointOverlayMarker >>> GuideOverlayTypeCongestion distance=");
                sb.append(strArr[0]);
                sb.append(strArr[1]);
                sb.append(" time =");
                sb.append(mTimeInfo);
                sb.append(", info: ");
                if (naviCongestionDetailInfo != null) {
                    str = naviCongestionDetailInfo.mTimeOfSeconds + ", " + NaviUtil.getTimeString(naviCongestionDetailInfo.mTimeOfSeconds);
                } else {
                    str = "";
                }
                sb.append(str);
                L.i(tag, L.formatWithEngine(sb.toString(), this.mEngineID));
            ...
            L.i(TAG, "getBizPointOverlayMarker >>> GuideOverlayTypeCongestion naviCongestionInfo is null");
            return -1;
        }
        return -1;
    }
