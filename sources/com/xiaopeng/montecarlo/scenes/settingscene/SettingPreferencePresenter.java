...
public class SettingPreferencePresenter extends BasePresenter implements ISettingContract.Presenter, BaseCallBack<SettingRequestParam, SettingResultData> {
    ...
    public void onCreate() {
        ...
        bindViewParam.mPublicationMapContent = this.mSettingDataProvider.getMapPublicationNumContent();
        bindViewParam.mInternetMapContent = this.mSettingDataProvider.getMapInternetNumContent();
        bindViewParam.mLaneMapContent = this.mSettingDataProvider.getMapLaneNumContent();
    ...
    public void resetDefaultSetting() {
        ...
        if (TBTManager.getInstance().isMiniMapEnable()) {
            ...
            intent2.setClassName(ContextUtils.getContext().getPackageName(), "com.xiaopeng.montecarlo.service.minimap.MiniMapService");
            intent2.setAction("com.xiaopeng.montecarlo.minimap.ACTION_SET_MAP_MODE");
            intent2.putExtra("map_mode", SettingWrapper.getCurrentMinimapMapMode());
            ContextUtils.getContext().startService(intent2);
        ...
        if (MapDataManager.getInstance().getMapDataOperator() != null) {
            MapDataManager.getInstance().getMapDataOperator().onAutoDownloadSwitchChanged(false);
        }
    ...
    protected void setMapDownloadAuto(boolean z) {
        if (MapDataManager.getInstance().getMapDataOperator() != null) {
            MapDataManager.getInstance().getMapDataOperator().onAutoDownloadSwitchChanged(z);
        }
    }
