...
public class RouteRequest implements Parcelable {
    ...
    public static RouteRequest createNaviRemoveViaRequest(XPPoiInfo xPPoiInfo, long j) {
        XPPoiInfo startPOIFromCurrent = TBTManager.getInstance().getStartPOIFromCurrent();
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            for (XPViaPoint xPViaPoint : list) {
                if (!NaviUtil.isSamePOI(xPPoiInfo, xPViaPoint.getPoiPoint()) && !xPViaPoint.getIsPassedVia()) {
        ...
        sb.append("createNaviRemoveViaRequest oldVia size= ");
