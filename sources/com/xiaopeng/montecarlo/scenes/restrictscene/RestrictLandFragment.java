...
public class RestrictLandFragment extends BaseFullScenes implements IRestrictContract.LogicView, DetailRestrictRulesLandView.IActionCallback {
    ...
    @BindView(R.id.restrict_detail_close)
    XImageButton mRestrictDetailClose;
    @BindView(R.id.restrict_detail_pageview)
    DetailRestrictRulesLandView mRestrictDetailPageview;
    ...
    public static BaseScene newInstance() {
        return new RestrictLandFragment();
    }
    ...
    private void updateTitleColor() {
        int i = this.mCityNum;
        ColorStateList colorStateList = getResources().getColorStateList(ThemeWatcherUtil.getColorResId(R.color.text_color_restrict_title));
        for (int i2 = 0; i2 < i; i2++) {
            ((TextView) this.mRestrictCityTitleList.getChildAt(i2)).setTextColor(colorStateList);
        }
    }
    ...
    public RestrictScenePresenter mo116getPresenter() {
    ...
    private void updateStatus(boolean z) {
        ...
        MapViewWrapper mapView = mainContext == null ? null : mainContext.getMapView();
        if (mapView == null) {
            return;
        }
        mapView.updateRestrictStatus(z);
    }
    ...
    public void updateTitleUnderLine() {
        ...
        this.mRestrictCityTitleList.post(new Runnable() { // from class: com.xiaopeng.montecarlo.scenes.restrictscene.RestrictLandFragment.1
            @Override // java.lang.Runnable
            public void run() {
                RestrictLandFragment.this.mUnderlineView.setVisibility(0);
                RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) RestrictLandFragment.this.mUnderlineView.getLayoutParams();
                View childAt = RestrictLandFragment.this.mRestrictCityTitleList.getChildAt(RestrictLandFragment.this.mSelectCityIndex);
                layoutParams.width = childAt.getWidth();
                layoutParams.setMarginStart(childAt.getLeft());
                RestrictLandFragment.this.mUnderlineView.setLayoutParams(layoutParams);
            }
        });
    }
    ...
    private void initTitleView(final int i, final XPGRestrictCity xPGRestrictCity) {
        ...
        textView.setOnClickListener(new View.OnClickListener() { // from class: com.xiaopeng.montecarlo.scenes.restrictscene.RestrictLandFragment.2
            @Override // android.view.View.OnClickListener
            public void onClick(View view) {
                if (view.isSelected()) {
                    return;
                }
                RestrictLandFragment.this.clearSelected();
                ((TextView) view).setSelected(true);
                RestrictLandFragment.this.mSelectRuleIndex = 0;
                RestrictLandFragment.this.mSelectCityIndex = i;
                RestrictLandFragment.this.updateTitleUnderLine();
                RestrictLandFragment.this.mRestrictPresenter.drawRestrictedAreaRules(i, 0);
                RestrictLandFragment.this.mRestrictDetailPageview.setRuleData(xPGRestrictCity, 0);
            }
        });
    ...
    private void clearRestrictedAreaRules() {
        this.mMapViewWrapper.getBizControlManager().clearAllItems(BizControlWrapper.BIZ_CONTROL_TYPE_AREA, 1014L);
        this.mMapViewWrapper.getBizControlManager().clearAllItems(BizControlWrapper.BIZ_CONTROL_TYPE_AREA, 1015L);
    }
    ...
    @Override // com.xiaopeng.montecarlo.views.DetailRestrictRulesLandView.IActionCallback
    ...
    public void onBuildScene() {
        ...
        MainContext mainContext = getMainContext();
        XImageButton xImageButton = this.mRestrictDetailClose;
