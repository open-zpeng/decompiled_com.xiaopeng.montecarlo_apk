...
public abstract class BaseNgpPresenter extends BasePresenter {
    ...
    public void updateDriveModeStatus(int i) {
        ...
        SRNaviManager.getInstance().openNaviLine(!SRUtil.isNgpStatus(i) && getLogicView().getMapView().isLaneMode() && SRNaviManager.getInstance().isLaneEnabled());
        if (getLogicView().getMapView().isLaneMode()) {
            SRNaviManager.getInstance().setDynamicViewMode(SRUtil.isNgpStatus(i));
        }
        getLogicView().refreshNgpPanelView();
    }
    ...
    public void changeDynamicLevelMode() {
        ...
        if (i == 0) {
            ...
            this.mSRDynamicLevelMode = 2;
        }
    }

    public void resetLevel() {
        SRNaviManager.getInstance().resetLevel();
    }
}
