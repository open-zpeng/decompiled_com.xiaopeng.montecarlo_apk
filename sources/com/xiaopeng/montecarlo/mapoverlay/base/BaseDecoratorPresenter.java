...
public class BaseDecoratorPresenter {
    ...
    public final XPPoiInfo findPointByIndex(int i) {
        if (i <= -1 || i >= this.mXPPointList.size()) {
            return null;
        }
        return this.mXPPointList.get(i);
    }
    ...
    public void updateXPPoiInfoList(@NonNull List<XPPoiInfo> list) {
        ...
        if (L.ENABLE) {
            L.Tag tag = this.mTag;
            L.d(tag, "updateXPPoiInfoList.size = " + size);
        }
        XPPointBaseData currentExpandedBizPoint = getCurrentExpandedBizPoint();
        if (L.ENABLE) {
            L.Tag tag2 = this.mTag;
            L.d(tag2, "updateXPPoiInfoList oldBizPoint = " + currentExpandedBizPoint);
        }
        ArrayList arrayList = new ArrayList();
        for (int i = 0; i < size; i++) {
            ...
            if (xPPoiInfo == null) {
                L.Tag tag3 = this.mTag;
                L.e(tag3, "updateXPPoiInfoList poiInfoList.get(i) == null , i = " + i);
        ...
        if (L.ENABLE) {
            L.Tag tag4 = this.mTag;
            L.d(tag4, "updateXPPoiInfoList oldPosition = " + this.mCurrentExpandedPosition + ", newPosition = " + findBizPoint);
        }
        if (currentExpandedBizPoint != null && findBizPoint == -1) {
            L.Tag tag5 = this.mTag;
            L.e(tag5, "updateXPPoiInfoList can't find point :" + currentExpandedBizPoint + " in " + arrayList.toString());
            if (L.ENABLE) {
