...
public class VoiceFullScenesUtil {
        ...
        if (canShowAnimation && view != null) {
            try {
                if (view.getVisibility() == 0) {
                    VuiFloatingLayerManager.show(view, false);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static void updateStatefulButtonState(View view, int i) {
        if (view instanceof IVuiElement) {
            if (L.ENABLE) {
                L.Tag tag = TAG;
                L.d(tag, "updateStatefulButtonState view.id = " + view.getId() + ", label =" + ((IVuiElement) view).getVuiLabel() + ", curIndex = " + i + ", view = " + view.toString());
            }
            VuiUtils.setStatefulButtonValue(view, i);
        }
    }
}
