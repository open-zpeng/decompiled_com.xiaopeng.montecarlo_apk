...
public abstract class DayNightFragment extends Fragment implements XpThemeSwitchReceiver.ThemeSwitchListener {
    ...
    protected int mCurrentSceneTheme = 0;
    ...
    private void noticeThemeSwitch() {
        int currentTheme = ThemeWatcherUtil.getCurrentTheme();
        if (L.ENABLE) {
            L.Tag tag = TAG;
    ...
    private void onThemeSwitchInternal(int i) {
        ...
        if (canSwitchDayNight) {
            this.mCurrentSceneTheme = i;
            onThemeSwitchDelegate(i);
        }
    }
    ...
    public void onThemeSwitchDelegate(int i) {
        if (L.ENABLE) {
            L.Tag tag = TAG;
            L.d(tag, L.formatDayNightSwitchLog("onThemeSwitchDelegate theme:" + i + ",who:" + this));
        }
    }
