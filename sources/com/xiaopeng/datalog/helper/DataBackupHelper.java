...
public class DataBackupHelper {
    ...
    public void initDir(String str, Context context) {
        File file = new File(DATA_FILE_DIR + str + "/" + context.getPackageName());
        if (file.exists()) {
            return;
        }
        file.mkdirs();
    ...
    public void deleteExpiredFile(final String str, Context context) {
        ...
        for (String str2 : list) {
            if (isExpiredFile(str2)) {
                arrayList.add(str2);
                boolean delete = new File(DATA_FILE_DIR + str + "/" + context.getPackageName() + "/" + str2).delete();
    ...
    public void writeFile(String str, byte[] bArr, Context context) {
        ...
        try {
            ...
            try {
                ...
                randomAccessFile.writeBytes(FDManager.LINE_SEPERATOR);
    ...
    private boolean isExpiredFile(String str) {
        int lastIndexOf = str.lastIndexOf(DebugConfiguration.DebugIntentActionExtra.POINT_SPLIT);
    ...
    private File getCurrentFile(final String str, Context context) {
        File[] listFiles = new File(DATA_FILE_DIR + str + "/" + context.getPackageName()).listFiles(new FilenameFilter() { // from class: com.xiaopeng.datalog.helper.DataBackupHelper.3
            @Override // java.io.FilenameFilter
    ...
    private boolean hasTodayFile(File file) {
        ...
        String formatDate10 = DateUtils.formatDate10(System.currentTimeMillis());
        return name.contains(formatDate10.substring(0, formatDate10.indexOf(DebugConfiguration.DebugIntentActionExtra.POINT_SPLIT)));
    }

    @NonNull
    private File createNewFile(String str, Context context) {
        String formatDate10 = DateUtils.formatDate10(System.currentTimeMillis());
        return new File(DATA_FILE_DIR + str + "/" + context.getPackageName() + "/" + str + DebugConfiguration.DebugIntentActionExtra.POINT_SPLIT + formatDate10 + ".log");
    }
    ...
    private String getTimeFromFile(String str) {
        return str.substring(str.lastIndexOf(DebugConfiguration.DebugIntentActionExtra.POINT_SPLIT) - 8, str.lastIndexOf("."));
    }
}
