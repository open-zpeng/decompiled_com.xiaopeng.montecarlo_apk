...
public class UpdateSceneAttributeTask extends BaseTask {
            ...
            if (arrayList.size() > 0) {
                ...
                if (size > 0) {
                    if (wholeSceneId.contains(VuiEngineImpl.mActiveSceneId)) {
                        VuiScene newVuiScene2 = getNewVuiScene(VuiEngineImpl.mActiveSceneId, currentTimeMillis);
                        newVuiScene2.setElements(arrayList);
                        setWholeSceneCache(VuiEngineImpl.mActiveSceneId, sceneCache2, arrayList);
                        VuiSceneManager.instance().sendSceneData(VuiSceneManager.TYPE_UPDATE, true, newVuiScene2);
                    }
                    for (int i2 = 0; i2 < size; i2++) {
                        String str = wholeSceneId.get(i2);
                        if (!VuiEngineImpl.mActiveSceneId.equals(str)) {
                            ...
                            VuiSceneManager.instance().sendSceneData(VuiSceneManager.TYPE_UPDATE, true, newVuiScene3);
                        }
                    }
                }
            }
    ...
    private VuiElement buildElement(VuiSceneCache vuiSceneCache, SoftReference<View> softReference, long j) {
        ...
        if (vuiElementById == null) {
            ...
            if (vuiElementById.isLayoutLoadable()) {
