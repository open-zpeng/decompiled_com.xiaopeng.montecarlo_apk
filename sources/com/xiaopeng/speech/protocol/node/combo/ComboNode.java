...
public class ComboNode extends SpeechNode<ComboListener> {
    ...
    public void closeMeditationMode(String str, boolean z) {
        ...
        SpeechClient.instance().getWakeupEngine().stopDialog();
        try {
            str2 = new JSONObject().put("tts", str).put("isAsrModeOffline", true).put("needTTS", z ? "true" : "false").toString();
        } catch (JSONException e) {
            e.printStackTrace();
            str2 = "";
        }
    ...
    public void openWaitMode(boolean z) {
        String str;
        try {
            str = new JSONObject().put("isAsrModeOffline", true).put("needTTS", z ? "true" : "false").toString();
        } catch (JSONException e) {
            e.printStackTrace();
            str = "";
        }
