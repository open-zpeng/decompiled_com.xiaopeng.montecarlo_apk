...
public class PhoneNode extends SpeechNode<PhoneListener> {
        ...
        if (!CarTypeUtils.isOverseasCarType() || CarTypeUtils.isE38EU()) {
            ...
            try {
                for (Contact.PhoneInfo phoneInfo : list) {
                    ...
                    jSONObject2.put(VuiConstants.ELEMENT_ID, phoneInfo.getId());
    ...
    public void syncCallLogs(List<CallLogs> list, String str) {
        ...
        try {
            for (CallLogs callLogs : list) {
                JSONObject jSONObject2 = new JSONObject();
                jSONObject2.put(VuiConstants.ELEMENT_ID, callLogs.getId());
                jSONObject2.put("name", callLogs.getName());
                jSONObject2.put(DebugConfiguration.DebugActionExtra.CRUISE_CONGESTION_TIME, callLogs.getTime());
    ...
    public void incomingCallRing(String str, String str2, boolean z) {
        ...
        SpeechClient.instance().getAgent().triggerIntentWithBinder(this.mBinder, "离线来电话", "来电话", "来电话", str4);
    ...
    private void disableWakeup() {
        if (!CarTypeUtils.isOverseasCarType()) {
            SpeechClient.instance().getWakeupEngine().disableWakeupWithInfo(this.mBinder, 2, "蓝牙电话", "通话中暂停服务，一会再叫我", 1);
            SpeechClient.instance().getWakeupEngine().disableWakeupWithInfo(this.mBinder, 4, "蓝牙电话", "通话中暂停服务，一会再叫我", 1);
                ...
                for (int i = 0; i < optJSONArray.length(); i++) {
                    ...
                    if (optJSONObject != null) {
                        ...
                        if (optJSONObject2 != null) {
                            phoneInfo.setId(optJSONObject2.optString(VuiConstants.ELEMENT_ID));
                        }
