...
public final class Hash234 extends CRC32Hash {
    ...
    public void calcHashes(byte[] bArr, int i) {
        int i2 = crcTable[bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR] ^ (bArr[i + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
        this.hash2Value = i2 & 1023;
        int i3 = i2 ^ ((bArr[i + 2] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8);
        this.hash3Value = 65535 & i3;
        this.hash4Value = ((crcTable[bArr[i + 3] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR] << 5) ^ i3) & this.hash4Mask;
    }
