...
public class TarArchiveEntry implements TarConstants, ArchiveEntry {
    ...
    private void parseTarHeader(byte[] bArr, ZipEncoding zipEncoding, boolean z) throws IOException {
        ...
        this.userName = z ? TarUtils.parseName(bArr, SearchResultCode.G_ERR_TELEPHONE, 32) : TarUtils.parseName(bArr, SearchResultCode.G_ERR_TELEPHONE, 32, zipEncoding);
        this.groupName = z ? TarUtils.parseName(bArr, MarkUtils.MARKER_ID_TIE_2, 32) : TarUtils.parseName(bArr, MarkUtils.MARKER_ID_TIE_2, 32, zipEncoding);
        this.devMajor = (int) TarUtils.parseOctalOrBinary(bArr, LocTrafficSignType.LOC_TRAFFIC_SIGN_MOTOR_VEHICLES, 8);
        this.devMinor = (int) TarUtils.parseOctalOrBinary(bArr, LocTrafficSignType.LOC_TRAFFIC_SIGN_EMERGENCY_LANE, 8);
        ...
        if (evaluateType == 2) {
            ...
            String parseName = z ? TarUtils.parseName(bArr, MarkUtils.MARKER_ID_BUBBLE_PASSED_VIA, 131) : TarUtils.parseName(bArr, MarkUtils.MARKER_ID_BUBBLE_PASSED_VIA, 131, zipEncoding);
            if (parseName.length() > 0) {
                this.name = parseName + MqttTopic.TOPIC_LEVEL_SEPARATOR + this.name;
            }
        } else {
            String parseName2 = z ? TarUtils.parseName(bArr, MarkUtils.MARKER_ID_BUBBLE_PASSED_VIA, 155) : TarUtils.parseName(bArr, MarkUtils.MARKER_ID_BUBBLE_PASSED_VIA, 155, zipEncoding);
