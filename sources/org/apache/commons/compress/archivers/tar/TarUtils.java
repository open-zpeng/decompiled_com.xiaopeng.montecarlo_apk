...
public class TarUtils {
    ...
    static final ZipEncoding FALLBACK_ENCODING = new ZipEncoding() { // from class: org.apache.commons.compress.archivers.tar.TarUtils.1
        ...
        public String decode(byte[] bArr) {
            ...
            for (byte b : bArr) {
                ...
                sb.append((char) (b & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR));
            }
            return sb.toString();
        }
    };
    ...
    private static long parseBinaryLong(byte[] bArr, int i, int i2, boolean z) {
        if (i2 < 9) {
            ...
            for (int i3 = 1; i3 < i2; i3++) {
                j = (j << 8) + (bArr[i + i3] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
            }
            if (z) {
    ...
    public static long computeCheckSum(byte[] bArr) {
        long j = 0;
        for (byte b : bArr) {
            j += b & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR;
        }
        return j;
    }
    ...
    public static boolean verifyCheckSum(byte[] bArr) {
        ...
        for (int i = 0; i < bArr.length; i++) {
            ...
            j2 += b & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR;
            j += b;
        }
