...
class DumpArchiveUtil {
    ...
    public static final int convert32(byte[] bArr, int i) {
        return (bArr[i + 3] << 24) + ((bArr[i + 2] << 16) & 16711680) + ((bArr[i + 1] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) + (bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
    }

    public static final int convert16(byte[] bArr, int i) {
        return ((bArr[i + 1] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) + 0 + (bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
    }
