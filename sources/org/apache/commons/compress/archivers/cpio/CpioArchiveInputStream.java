...
public class CpioArchiveInputStream extends ArchiveInputStream implements CpioConstants {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.entry.getFormat() == 2) {
            for (int i3 = 0; i3 < readFully; i3++) {
                this.crc += bArr[i3] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR;
            }
        }
    ...
    public static boolean matches(byte[] bArr, int i) {
        ...
        if (bArr[0] == 113 && (bArr[1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) == 199) {
            return true;
        }
        if (bArr[1] == 113 && (bArr[0] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) == 199) {
            return true;
        }
