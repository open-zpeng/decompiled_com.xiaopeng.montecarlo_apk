...
public class SimpleBase64Encoder {
    ...
    public static String encode(byte[] bArr) {
        ...
        int i = 0;
        while (length >= 3) {
            stringBuffer.append(to64(((bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 16) | ((bArr[i + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8) | (bArr[i + 2] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR), 4));
            i += 3;
            length -= 3;
        }
        if (length == 2) {
            stringBuffer.append(to64(((bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8) | (bArr[i + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR), 3));
        }
        if (length == 1) {
            stringBuffer.append(to64(bArr[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR, 2));
        }
