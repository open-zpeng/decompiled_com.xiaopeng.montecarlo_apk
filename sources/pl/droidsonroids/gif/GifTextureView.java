...
public class GifTextureView extends TextureView {
    ...
    /* JADX WARN: Type inference failed for: r0v0, types: [android.util.TypedValue, java.lang.Object[]] */
    /* JADX WARN: Type inference failed for: r1v0, types: [int, java.lang.reflect.Method] */
    private static InputSource findSource(TypedArray typedArray) {
        ?? typedValue = new TypedValue();
        if (typedArray.invokeMethod(R.styleable.GifTextureView_gifSource, typedValue) == null) {
            return null;
        }
        if (((TypedValue) typedValue).resourceId != 0) {
            String resourceTypeName = typedArray.getResources().getResourceTypeName(((TypedValue) typedValue).resourceId);
            if (GifViewUtils.SUPPORTED_RESOURCE_TYPE_NAMES.contains(resourceTypeName)) {
                return new InputSource.ResourcesSource(typedArray.getResources(), ((TypedValue) typedValue).resourceId);
            }
            if (!TypedValues.Custom.S_STRING.equals(resourceTypeName)) {
                throw new IllegalArgumentException("Expected string, drawable, mipmap or raw resource type. '" + resourceTypeName + "' is not supported");
            }
        }
        return new InputSource.AssetSource(typedArray.getResources().getAssets(), ((TypedValue) typedValue).string.toString());
    }
