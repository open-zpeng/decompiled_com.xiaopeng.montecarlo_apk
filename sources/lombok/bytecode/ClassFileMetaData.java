...
/* loaded from: 地图_V1.10.0_20240112105442_Release.apk:lombok/bytecode/ClassFileMetaData.SCL.lombok */
public class ClassFileMetaData {
    ...
    private String decodeString(int pos, int size) {
        ...
        while (pos < end) {
            int i = pos;
            pos++;
            int first = this.byteCode[i] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR;
            if (first < 128) {
                int i2 = length;
                length++;
    ...
    private int read32(int pos) {
        return ((this.byteCode[pos] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 24) | ((this.byteCode[pos + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 16) | ((this.byteCode[pos + 2] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8) | (this.byteCode[pos + 3] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
    }
    ...
    private int readValue(int position) {
        return ((this.byteCode[position] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR) << 8) | (this.byteCode[position + 1] & AIMGroupService.AIM_MAX_GROUP_MEMBER_CURSOR);
    }
}
